...
func runSkillImport(args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: gostaff skill import <skill-dir> [dest-dir]")
		os.Exit(1)
	}
...
func runSkillSearch(args []string) error {
	if len(args) == 0 {
		return fmt.Errorf("usage: gostaff skill search <query>")
	}
...
func runSkillInstall(args []string) error {
	...
	// Write to temp file
	tmp, err := os.CreateTemp("", "gostaff-skill-*")
	if err != nil {
...
func runSkillInstallFromGitHub(target, destDir string) error {
	...
	tmp, err := os.CreateTemp("", "gostaff-gh-*.tar.gz")
	if err != nil {
		return fmt.Errorf("creating temp file: %w", err)
	}
	tmpPath := tmp.Name()
...
func extractZip(src string) (string, error) {
	...
	dir, err := os.MkdirTemp("", "gostaff-skill-extract-*")
	if err != nil {
		return "", err
	}
...
func runSkillInit(args []string) error {
	...
	if len(filtered) == 0 {
		return fmt.Errorf("usage: gostaff skill init [--plugin] <name>")
	}
	...
	if plugin {
		...
		fmt.Printf("\nLoad it with: gostaff skill import ./%s\n", name)
